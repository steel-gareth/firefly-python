...
from emcees_prod_testing_5 import MoreConflicting, AsyncMoreConflicting, DefaultAioHttpClient
from emcees_prod_testing_5._utils import is_dict
...
logging.getLogger("emcees_prod_testing_5").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[MoreConflicting]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with MoreConflicting(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncMoreConflicting]:
